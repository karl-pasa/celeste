...
<head>
    ...
    <title>@yield('title', 'Verify a document') — CELESTE | {{ config('celeste.institution.short') }}</title>
    <meta name="description" content="Check whether a document issued by {{ config('celeste.institution.name') }} is authentic. Scan its QR code or enter its serial number.">
...
<div class="celeste-field">
    <nav class="container py-3 d-flex align-items-center justify-content-between">
        <a href="{{ route('home') }}" class="d-flex align-items-center gap-2 text-decoration-none">
            ...
            <span>
                ...
                <span class="d-block" style="color:rgba(255,255,255,.6);font-size:.6875rem">{{ config('celeste.institution.short') }}</span>
            </span>
        </a>
        <div class="d-flex align-items-center gap-2">
            <a href="{{ route('verify.scanner') }}" class="btn btn-sm" style="background:rgba(255,255,255,.12);color:#fff;border:0">
                <i class="bi bi-qr-code-scan"></i> <span class="d-none d-sm-inline">Scan</span>
            </a>
            @auth
                <a href="{{ auth()->user()->isRegistrar() ? route('registrar.dashboard') : route('student.dashboard') }}"
    ...
    <footer class="container pb-4 text-center" style="color:rgba(255,255,255,.5);font-size:.75rem">
        &copy; {{ date('Y') }} {{ config('celeste.institution.name') }}, {{ config('celeste.institution.campus') }}.
