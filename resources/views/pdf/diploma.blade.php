<!DOCTYPE html>
{{-- DomPDF renders a limited CSS subset: tables and inline styles only, no flex or grid. --}}
<html>
<head>
    <meta charset="utf-8">
    <style>
...
<div class="sheet">
    <div class="border-outer">
        <div style="text-align:center">
            ...
            <div class="name">{{ $payload['full_name'] }}</div>

            <div class="lead">who has satisfactorily completed all the requirements prescribed for the degree of</div>

            <div class="degree">{{ mb_strtoupper($payload['program']) }}</div>

            @if (!empty($payload['major']))
                <div class="lead">major in {{ $payload['major'] }}</div>
            @endif

            @if (!empty($payload['latin_honor']))
                <div class="honor">{{ $payload['latin_honor'] }}</div>
            @endif
            ...
            <div class="lead" style="margin-top:10px">
                Given at {{ $payload['campus'] }}, Philippines, this
                {{ \Illuminate\Support\Carbon::parse($payload['issued_on'])->format('jS \d\a\y \o\f F, Y') }}.
            </div>
        </div>
        ...
        <table width="100%" style="margin-top:36px" cellpadding="0" cellspacing="0">
            <tr>
                ...
                <td width="50%" align="center">
                    <div class="sig-line">
                        <div class="sig-name">{{ config('celeste.officials.president') }}</div>
        ...
        <div class="footer-left">
            Verify at {{ config('app.url') }}/verify &nbsp;·&nbsp; Student No. {{ $payload['student_number'] }}<br>
            <span class="hash">SHA-256 {{ $certificate->content_hash }}</span>
        </div>
