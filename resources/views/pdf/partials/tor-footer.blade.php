{{--
    Rating legend, validity note, signatories, and the verification block.

    The legend, the rating marks and the note share one bordered box with no
    rules between them: on the printed form they read as a single block of
    small print, not as three stacked panels.

    The QR code sits in the "Not valid without school seal" cell. That is the
    one part of the footer with room for it, and it puts the verification mark
    beside the statement about the seal — the two things a verifier looks at.
--}}
...
<table class="bx" style="border-top:0">
    <tr>
        <td style="width:34%; border-right:.7pt solid #000; padding:0">
            <table>
                ...
                <tr><td style="padding:0 1.4mm 1mm">
                    ...
                    <div style="font-size:6pt">{{ $p['prepared_by'] ?? '' }}</div>
                </td></tr>
                <tr><td class="lbl" style="border-top:.7pt solid #000; padding:.7mm 1.4mm">Reviewed by:</td></tr>
                <tr><td style="padding:0 1.4mm 1mm">
                    <div style="height:6mm"></div>
                    <div class="sig-line"></div>
                    <div style="font-size:6pt">{{ $p['reviewed_by'] ?? '' }}</div>
                </td></tr>
            </table>
        </td>
        ...
        <td style="width:36%; border-right:.7pt solid #000; vertical-align:bottom; padding-bottom:2mm">
            ...
            <div class="reg-name">{{ config('celeste.officials.registrar', 'JOJI B. MIRAÑA, EdD') }}</div>
            <div class="reg-role">University Registrar</div>
        </td>

        {{-- Verification block. The QR carries the whole claim, so it is given
             the space; the serial is printed beneath for anyone typing it in
             by hand when a code will not scan. --}}
        <td style="width:30%; text-align:center; padding:1.5mm 1mm">
            <div style="font-size:7pt">Not valid without school seal</div>

            @if (!empty($qr))
                <img src="{{ $qr }}" style="width:22mm; height:22mm; margin-top:1.2mm">
            @else
            ...
            <div class="mono" style="font-size:6.2pt; margin-top:.6mm">{{ $certificate->serial_number }}</div>
            <div style="font-size:5.4pt; color:#555">Scan to verify</div>

            <div style="font-size:6.4pt; margin-top:1.2mm">
                Page <span style="border-bottom:.5pt solid #000; padding:0 2.5mm">{{ $page }}</span>
                of <span style="border-bottom:.5pt solid #000; padding:0 2.5mm">{{ $pages }}</span>
            </div>
        </td>
    </tr>
</table>
